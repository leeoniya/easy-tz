...
// impls/10-audited-rules/index.ts
...
function predictedTransitions(cls, year) {
  ...
  if (Date.UTC(year, 0, 1) !== YEAR_START)
    return [];
  const out = [];
...
function auditZone(zone, cls, yearStart, yearEnd, year) {
  let zdt = Temporal.Instant.fromEpochMilliseconds(yearStart).toZonedDateTimeISO(zone);
  if (parseOffset(zdt.offset) !== resolveClass(cls, yearStart, YEAR_START, STEP_MS).offMin)
    return false;
  const predicted = predictedTransitions(cls, year);
  let i = 0;
  for (;; ) {
    const next = zdt.getTimeZoneTransition("next");
    if (next === null || next.epochMilliseconds >= yearEnd)
      break;
...
function compute(timestamp) {
  ...
  if (hasTemporal && timestamp < HISTORY_TO_MS) {
    ...
    const out2 = new Array(zones.length);
    for (let z = 0;z < zones.length; z++)
      out2[z] = liveInfo(zones[z], classIdx[z], instant);
    return out2;
  }
  ...
  if (recovered.size > 0) {
    ...
    for (const z of recovered)
      out[z] = liveRecovered(zones[z], instant);
  }
  return out;
}
...
function computeCurrent(timestamp) {
  if (recovered === null)
    init();
