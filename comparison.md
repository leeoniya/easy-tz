...
Prefix: 📦 pure data/library package · UI components: ⚛️ React · 💚 Vue · 🅰️ Angular (🟨 would mark a framework-agnostic one).
...
| ⚛️ `react-timezone-select@3.3.3` (spacetime + timezone-soft) | popular React picker component — the exact UI this function feeds | EET / EEST | ❌ Disqualified despite passing the abbreviation gate (its `timezone-soft` dependency ships a curated per-zone abbr dataset — the same artifact this repo generates): its default list has **78 zones** (of 445) — `Europe/Kyiv` and `America/Denver` aren't in it (the gate passes only by calling `timezone-soft` directly); the curated data is stale for common zones — Cairo in summer renders **no abbreviation** (no EEST entry despite Egypt's 2023 DST return) and spacetime puts Volgograd at +04:00 (rejoined MSK +03:00 in 2020); plus offset bugs in the exotic corners (Lord Howe DST reports +11:30; reality +11:00). 124.3 KB minified (bundlephobia, excl. react-select). |
| 💚 `vue-timezone-select@1.0.3` (spacetime + timezone-soft) | Vue port of react-timezone-select | EET / EEST | ❌ Disqualified: same stack and failure modes as react-timezone-select above (stale `timezone-soft` abbrs, spacetime offset bugs) on even older pinned versions (`timezone-soft@^1.3.1`, unmaintained since 2022); its ~107-zone default list still uses the pre-2022 `Europe/Kiev` spelling. 136.5 KB minified (bundlephobia). |
| 🅰️ `moment-timezone-picker@9.0.0` (ng-select + moment-timezone) | the most-downloaded Angular timezone picker | EET / EEST | ❌ Disqualified as a *separate* candidate: abbreviations come wholesale from its bundled `moment-timezone` dependency via `zoneAbbr()` — the data verdict is identical to the moment-timezone row above, except it pins the older 0.5.x line. 1022 KB minified (bundlephobia, incl. ng-select). |
...
| 📦 `@internationalized/date@3.12.2` | Adobe/React-Aria date library; ubiquitous in the React ecosystem | GMT+2 / GMT+3 | ❌ Disqualified: `DateFormatter` is a thin `Intl.DateTimeFormat` wrapper — same `short`-name passthrough as luxon. |
| 📦 `@js-temporal/polyfill@0.5.1` | spec-track Temporal polyfill | GMT+2 / GMT+3 | ❌ Disqualified: Temporal has no abbreviation concept (offsets only, by spec); `toLocaleString` delegates to Intl `short`. |
| 📦 `temporal-polyfill@1.0.1` (fullcalendar) | lightweight production Temporal polyfill | GMT+2 / GMT+3 | ❌ Disqualified: same by-design gap — `ZonedDateTime.offset` gives "+03:00", never a letter abbreviation. |
...
| 📦 `countries-and-timezones@3.9.0` | minimal static zone/country metadata, popular for pickers | (none) | ❌ Disqualified: per-zone static `utcOffset`/`dstOffset` pairs only — no timestamp parameter (can't tell which offset applies when) and no abbreviation field at all. |
