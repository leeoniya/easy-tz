...
export function writeTableSet(
  variant: Variant,
  files: { classes: string; schedule: string; history: string; abbrfix: string }
): Variant {
  ...
  writeFileSync(new URL('history.ts', dir), files.history);
  writeFileSync(new URL('abbrfix.ts', dir), files.abbrfix);
